...
function Navbar({ view, setView }) {
  ...
  return (
    <div style={{
      background: "#1a1310", position: "sticky", top: 0, zIndex: 99,
      ...
      <nav style={{ display: "flex", gap: 4, flex: 1 }}>
        {nav.map(n => (
          <button key={n.id} onClick={() => setView(n.id)} style={{
            ...
            fontWeight: view === n.id ? 800 : 500, transition: "all 0.15s",
...
function BillingView({ products, filtered, category, setCategory, search, setSearch, cart, setCart, addToCart, updateQty, setQtyPreset, cartTotal, cartSubtotal, discountAmt, discount, setDiscount, customerForm, setCustomerForm, checkoutBill }) {
  const [popup, setPopup] = useState(null); // product being configured
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, width: 100, flexShrink: 0 }}>
          {["All", "Dairy", "Sweets", "Snacks", "Tandoor"].map(c => (
            <button key={c} onClick={() => setCategory(c)} style={{
              ...
              gap: 5, padding: "12px 8px", borderRadius: 14, border: "2px solid",
              ...
              fontSize: 12, fontWeight: category === c ? 800 : 500,
      ...
      {/* ── Right panel: summary + discount + bill ── */}
      <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e0d8", overflow: "hidden", position: "sticky", top: 80 }}>
        ...
        {/* Selected items list — simple, no controls */}
        <div style={{ maxHeight: 260, overflowY: "auto" }}>
          ...
          {cart.map((item, i) => (
            <div key={item.id} onClick={() => openPopup(item)} style={{
              ...
              padding: "10px 14px", borderTop: i > 0 ? "1px solid #f0ebe4" : "none",
      ...
      {popup && (
        <>
          ...
          <div style={{
            ...
            background: "#fff", borderRadius: 20, padding: 28, width: 340, zIndex: 201,
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(popup.unit === "piece" ? [1, 2, 5, 10] : [0.25, 0.5, 1, 2]).map(p => (
                  ...
                  }}>{popup.unit === "piece" ? p : formatQty(p, popup.unit)}</button>
                ))}
              </div>
            </div>
...
function ProductsView({ products, setProducts }) {
  ...
  const save = () => {
    ...
    if (editing) {
      setProducts(prev => prev.map(p => p.id === editing ? { ...p, ...form, price: +form.price, cost: +form.cost } : p));
      setEditing(null);
    } else {
      setProducts(prev => [...prev, { ...form, id: "p" + Date.now(), price: +form.price, cost: +form.cost }]);
    }
    setForm({ name: "", category: "Dairy", price: "", cost: "", unit: "kg" });
  };
  const edit = (p) => { setEditing(p.id); setForm({ name: p.name, category: p.category, price: p.price, cost: p.cost, unit: p.unit }); };
    ...
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, alignItems: "start" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e0d8", padding: 20, position: "sticky", top: 80 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={save} style={{ flex: 1, padding: "11px", background: "#1a1310", color: "#f59e0b", border: "none", borderRadius: 10, fontWeight: 800, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            <Icon name="save" size={16} /> {editing ? "Update" : "Add Product"}
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["today", "month", "all"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              padding: "8px 18px", borderRadius: 20, fontWeight: 700, fontSize: 13, cursor: "pointer", border: "1.5px solid",
              borderColor: filter === f ? "#f59e0b" : "#e5e0d8",
              background: filter === f ? "#f59e0b" : "#fff",
              color: filter === f ? "#1a1310" : "#8a7e6e",
            }}>{labels[f]}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e5e0d8", overflow: "hidden" }}>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", color: "#c9b9a8", padding: "40px 0", fontSize: 14 }}>Koi bill nahi {labels[filter].toLowerCase()} mein</div>
        )}
        {filtered.map((b, i) => (
          <div key={b.id} style={{ display: "flex", alignItems: "center", padding: "13px 20px", borderTop: i > 0 ? "1px solid #f0ebe4" : "none", gap: 16, flexWrap: "wrap" }}>
            ...
            {b.customer?.name && (
              <div style={{ fontSize: 12, color: "#4a3f35" }}>👤 {b.customer.name}</div>
            )}
            <div style={{ fontSize: 12, color: "#8a7e6e" }}>{b.items?.length} items</div>
            {b.discountPct > 0 && (
              <div style={{ fontSize: 12, color: "#f59e0b", fontWeight: 700 }}>🏷️ {b.discountPct}% off</div>
            )}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
        {[
          { label: "Today Sales", value: formatINR(todaySales), icon: "cart", color: "#2563eb", sub: `${todayBills.length} bills` },
          { label: "Today Profit", value: formatINR(todayProfit), icon: "profit", color: "#16a34a", sub: `${todaySales > 0 ? Math.round((todayProfit / todaySales) * 100) : 0}% margin` },
          { label: "Total Sales", value: formatINR(totalSales), icon: "analytics", color: "#7c3aed", sub: `${bills.length} bills ever` },
          { label: "Total Profit", value: formatINR(totalProfit), icon: "profit", color: "#ea580c", sub: `${totalSales > 0 ? Math.round((totalProfit / totalSales) * 100) : 0}% margin` },
        ].map(k => (
          <div key={k.label} style={{ background: "#fff", borderRadius: 16, padding: "20px", border: "1px solid #e5e0d8" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 160, overflowX: "auto" }}>
            {dailyData.map(([date, v]) => (
              <div key={date} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, minWidth: 28 }}>
                ...
                <div style={{ width: 20, borderRadius: "4px 4px 0 0", background: "linear-gradient(to top, #2563eb, #60a5fa)", height: Math.max(4, (v.sales / maxSales) * 130) }} title={`₹${v.sales}`} />
                <div style={{ width: 20, borderRadius: "4px 4px 0 0", background: "linear-gradient(to top, #16a34a, #4ade80)", height: Math.max(2, (v.profit / maxSales) * 130) }} title={`Profit: ₹${v.profit}`} />
        ...
        <div style={{ maxHeight: "70vh", overflowY: "auto" }}>
          ...
          {filtered.map(c => (
            <button key={c.phone} onClick={() => setSelected(c)} style={{
              width: "100%", textAlign: "left", padding: "13px 16px", borderBottom: "1px solid #f0ebe4",
